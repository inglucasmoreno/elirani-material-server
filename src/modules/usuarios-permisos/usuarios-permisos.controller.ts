...
export class UsuariosPermisosController {
  ...
  @Get('/:id')
  async permisoPorId(@Res() res, @Param('id') id: number): Promise<UsuariosPermisos> {
  ...
  @UseGuards(JwtAuthGuard)
  @Get('/')
  async listarPermisos(@Res() res, @Query() query): Promise<UsuariosPermisos[]> {
  ...
  @UseGuards(JwtAuthGuard)
  @Post('/')
  async crearPermiso(@Res() res, @Body() permisosDTO: UsuariosPermisosDTO): Promise<UsuariosPermisos> {
